...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h3 class="fw-bold text-nowrap"><i class="fas fa-envelope-open-text me-2"></i> Data Surat Undangan</h3>
    @can('surat_undangan_create')
        <a class="btn btn-success shadow-sm" href="{{ route('admin.surat-undangan.create') }}">
...
<div class="card border-0 shadow-sm">
    <div class="card-body">
        <div class="table-responsive">
            <table class="modern-table datatable datatable-SuratUndangan" style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th><i class="fas fa-bullseye me-1"></i>Tujuan (Yth.)</th>
                        <th><i class="fas fa-calendar-check me-1"></i>Agenda</th>
...
$(function () {
    ...
    // Ubah can() sesuai role permission milikmu, jika tidak pakai spatie biarkan saja
    @can('surat_undangan_delete')
    let deleteButtonTrans = '{{ trans('global.datatables.delete') }}';
    let deleteButton = {
        text: deleteButtonTrans,
        url: "{{ route('admin.surat-undangan.massDestroy') }}", 
        className: 'btn-danger',
        action: function (e, dt, node, config) {
            var ids = $.map(dt.rows({ selected: true }).data(), function (entry) {
                return entry.id; // Mengambil ID dari row yang terpilih
            });
            ...
            Swal.fire({
                ...
                text: "Data arsip surat undangan yang dipilih akan dihapus permanen!",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        $('.datatable-SuratUndangan').DataTable().ajax.reload();
    ...
    let table = $('.datatable-SuratUndangan').DataTable({
        processing: true,
        serverSide: true,
        ...
        columns: [
            ...
            { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false },
            {
                ...
                render: function(data, type, row) {
                    if (!data) {
                        return '<span class="badge bg-danger bg-opacity-10 text-danger border border-danger-subtle px-2 py-1 small"><i class="fas fa-clock me-1"></i>Menunggu Nomor</span>';
                    }
                    return '<div class="fw-bold text-primary">' + data + '</div>'
                         + '<small class="text-muted">' + (row.hal_surat || 'Surat Undangan') + '</small>';
                }
            },
        ...
        order: [[ 2, 'desc' ]], // Default diurutkan berdasarkan Nomor/Tanggal terbaru
        pageLength: 50,
        language: {                                                    
    ...
    // 4. Handle Enable/Disable Tombol Ekspor Saat Ceklis Berubah
    ...
    // 5. Tangkap event hapus satuan (Single Delete) via Delegasi ke class form-delete
    $(document).on('submit', '.form-delete', function(e) {
        ...
        Swal.fire({
            ...
            text: "Data surat undangan yang dihapus tidak dapat dikembalikan!",
            icon: 'warning',
            showCancelButton: true,
...
// 6. [Opsional] Fungsi SweetAlert untuk Input Nomor Surat via AJAX (jika kamu pakai sistem serupa dengan surat tugas)
function openNomorModal(id, currentNomor) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: "/admin/surat-undangan/" + id + "/update-nomor", 
                type: 'POST',
                data: {
                ...
                success: function(response) {
                    Swal.fire({ icon: 'success', title: 'Berhasil!', text: 'Nomor surat berhasil disimpan.', timer: 1500, showConfirmButton: false });
                    $('.datatable-SuratUndangan').DataTable().ajax.reload(null, false);
                },
                error: function(xhr) {
