...
<div class="d-flex justify-content-between align-items-center mb-3">
    <div>
        ...
        <small class="text-muted">Nomor: <strong>{{ $surat->nomor_surat }}</strong></small>
    </div>
...
<div class="row g-3">
    ...
    <div class="col-xl-5 col-lg-6">
        <div class="card border-0 shadow-sm">
            <div class="card-body">
                <form id="suratForm"
                      action="{{ route('admin.surat-undangan.update', $surat->id) }}"
                      method="POST">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="d-flex gap-3">
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="mode_tujuan"
                                       id="mode_biasa" value="biasa"
                                       {{ !$modeLampiran ? 'checked' : '' }}
                                       onchange="toggleTujuan()">
                                <label class="form-check-label" for="mode_biasa">Tulis di Surat</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="mode_tujuan"
                                       id="mode_lampiran" value="lampiran"
                                       {{ $modeLampiran ? 'checked' : '' }}
                                       onchange="toggleTujuan()">
                                <label class="form-check-label" for="mode_lampiran">Gunakan Lampiran</label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="container_tujuan_biasa" class="mb-3 {{ $modeLampiran ? 'd-none' : '' }}">
                        ...
                        <select name="tujuan_surat[]" id="tujuan_surat" multiple="multiple">
                            @foreach($tujuanPresets as $preset)
                                <option value="{{ $preset }}"
                                    {{ (!$modeLampiran && in_array($preset, $surat->tujuan_list)) ? 'selected' : '' }}>
                                    {{ $preset }}
                                </option>
                            @endforeach
                            {{-- Tujuan custom yang tidak ada di preset --}}
                            @if(!$modeLampiran)
                    ...
                    <div id="container_tujuan_lampiran" class="mb-3 {{ !$modeLampiran ? 'd-none' : '' }}">
                        <label class="form-label">Daftar Penerima <small class="text-muted">(format: Nama - Jabatan)</small></label>
                        <textarea name="lampiran_content" class="form-control font-monospace" rows="8">{{ old('lampiran_content', $surat->lampiran_content) }}</textarea>
                    </div>
                    ...
                    @php
                        // Parsing hari_tanggal_acara kembali ke format date untuk input
                        // Data di DB: "Senin, 05 Mei 2025" — kita perlu rawnya
                        // Simpan tanggal_acara_raw di DB agar bisa di-edit ulang
                    @endphp
                    ...
                    <div class="mb-3">
                        ...
                        <small class="text-muted">Tertulis di surat: <strong>{{ $surat->hari_tanggal_acara }}</strong></small>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="tempat_acara" id="tempat_acara" class="form-control">
                            ...
                            @foreach($tempatPresets as $tempat)
                                <option value="{{ $tempat }}"
                                    {{ old('tempat_acara', $surat->tempat_acara) == $tempat ? 'selected' : '' }}>
                                    {{ $tempat }}
                                </option>
                            @endforeach
                    ...
                    <div class="mb-3">
                        ...
                        <select name="penandatangan_index" id="penandatangan_index" class="form-select">
                            @foreach($penandatangans as $index => $p)
                                <option value="{{ $index }}"
                                    {{ old('penandatangan_index', 0) == $index ? 'selected' : '' }}>
                                    {{ $p['jabatan'] }} — {{ $p['nama'] }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" id="use_ttd"
                                   name="use_ttd" value="1" style="width:2.5em;height:1.5em;">
                    ...
                    <div class="d-grid gap-2">
                        ...
                        <button type="submit" class="btn btn-warning btn-lg text-white">
                            <i class="fas fa-save me-2"></i> Simpan Perubahan & Download PDF
                        </button>
                    </div>
                </form>
            </div>
...
function updatePreview() {
    ...
    $.ajax({
        ...
        error: function (xhr) {
            var msg = xhr.status === 422 ? 'Lengkapi field yang wajib diisi (*).' : 'Gagal memuat preview.';
