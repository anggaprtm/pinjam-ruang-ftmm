...
<div class="card mb-3"><div class="card-header d-flex justify-content-between align-items-center"><strong>Import Proker (Excel/CSV)</strong><a href="{{ route('admin.ormawa-plans.template') }}" class="btn btn-sm btn-outline-secondary">Download Template</a></div><div class="card-body">
<form method="POST" action="{{ route('admin.ormawa-plans.items.import', $ormawaPlan->id) }}" enctype="multipart/form-data">@csrf
<div class="row g-2 align-items-end"><div class="col-md-10"><label>File (.xlsx/.xls/.csv)</label><input type="file" name="file" class="form-control" required></div><div class="col-md-2"><button class="btn btn-dark w-100">Import</button></div></div>
</form>
</div></div>
...
<td><textarea class="form-control form-control-sm" name="deskripsi_rencana" rows="1">{{ $item->deskripsi_rencana }}</textarea></td>
...
<td class="d-flex gap-1"><button class="btn btn-xs btn-primary">Update</button></form><form method="POST" action="{{ route('admin.ormawa-plans.items.destroy', [$ormawaPlan->id, $item->id]) }}">@csrf @method('DELETE')<button class="btn btn-xs btn-danger" onclick="return confirm('Hapus item?')">Hapus</button></form></td></tr>
@empty<tr><td colspan="7" class="text-center text-muted">Belum ada item.</td></tr>@endforelse
</tbody></table>
</div></div>
@endsection
