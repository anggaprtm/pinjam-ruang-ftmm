...
<div class="card form-card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h4 class="mb-0">
            <i class="fas fa-edit me-2 text-primary"></i> Edit Agenda Fakultas
        </h4>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route('admin.agenda-fakultas.update', $agendaFakultas->id) }}">
            @csrf
            @method('PUT')
            ...
            <div class="form-group mb-3">
                ...
                <input class="form-control {{ $errors->has('judul') ? 'is-invalid' : '' }}"
                       ...
                       value="{{ old('judul', $agendaFakultas->judul) }}"
                       required>
                @if($errors->has('judul'))
            ...
            <div class="form-group mb-3">
                ...
                <textarea class="form-control {{ $errors->has('deskripsi') ? 'is-invalid' : '' }}"
                          name="deskripsi" id="deskripsi" rows="3">{{ old('deskripsi', $agendaFakultas->deskripsi) }}</textarea>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control {{ $errors->has('kategori') ? 'is-invalid' : '' }}"
                            ...
                            @foreach($kategoriOptions as $opt)
                                <option value="{{ $opt }}"
                                    {{ old('kategori', $agendaFakultas->kategori) === $opt ? 'selected' : '' }}>
                                    {{ $opt }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control {{ $errors->has('warna') ? 'is-invalid' : '' }}"
                            ...
                            @foreach($warnaOptions as $hex => $label)
                                <option value="{{ $hex }}"
                                    {{ old('warna', $agendaFakultas->warna) === $hex ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <div class="mt-2 d-flex align-items-center gap-2">
                            <span id="warnaPreview"
                                  style="display:inline-block;width:20px;height:20px;border-radius:4px;
                                         background:{{ old('warna', $agendaFakultas->warna) }};
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('tanggal_mulai') ? 'is-invalid' : '' }}"
                               ...
                               value="{{ old('tanggal_mulai', $agendaFakultas->tanggal_mulai->format('Y-m-d')) }}"
                               required>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('tanggal_selesai') ? 'is-invalid' : '' }}"
                               ...
                               value="{{ old('tanggal_selesai', $agendaFakultas->tanggal_selesai?->format('Y-m-d')) }}">
            ...
            {{-- SEHARIAN / WAKTU --}}
            @php $isAllDay = old('is_all_day', $agendaFakultas->is_all_day ? '1' : '0'); @endphp
            <div class="form-group mb-3">
                <div class="form-check">
                    <input type="hidden" name="is_all_day" value="0">
                    <input class="form-check-input" type="checkbox"
                           name="is_all_day" id="is_all_day" value="1"
                           {{ $isAllDay === '1' || $isAllDay === true ? 'checked' : '' }}
            ...
            <div class="row mb-3" id="waktuRow"
                 style="{{ ($isAllDay === '1' || $isAllDay === true) ? 'display:none;' : '' }}">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('waktu_mulai') ? 'is-invalid' : '' }}"
                               ...
                               value="{{ old('waktu_mulai', $agendaFakultas->waktu_mulai) }}">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('waktu_selesai') ? 'is-invalid' : '' }}"
                               ...
                               value="{{ old('waktu_selesai', $agendaFakultas->waktu_selesai) }}">
                ...
                <div class="col-md-6">
                    <div class="form-check">
                        ...
                        <input class="form-check-input" type="checkbox"
                               ...
                               {{ old('tampil_di_signage', $agendaFakultas->tampil_di_signage ? '1' : '0') === '1' ? 'checked' : '' }}>
                ...
                <div class="col-md-6">
                    <div class="form-check">
                        ...
                        <input class="form-check-input" type="checkbox"
                               ...
                               {{ old('tampil_countdown', $agendaFakultas->tampil_countdown ? '1' : '0') === '1' ? 'checked' : '' }}>
            ...
            <div class="form-group mb-4">
                <label class="form-label" for="urutan">
                    Urutan Tampil <span class="text-muted">(angka lebih kecil = lebih atas)</span>
                </label>
                <input class="form-control {{ $errors->has('urutan') ? 'is-invalid' : '' }}"
                       type="number" name="urutan" id="urutan"
                       value="{{ old('urutan', $agendaFakultas->urutan) }}"
            ...
            {{-- INFO: dibuat oleh & waktu --}}
            <div class="alert alert-light border text-muted small mb-4">
                <i class="fas fa-info-circle me-1"></i>
                Dibuat oleh <strong>{{ $agendaFakultas->creator->name ?? '-' }}</strong>
                pada {{ $agendaFakultas->created_at->translatedFormat('d F Y, H:i') }} WIB.
                Terakhir diperbarui {{ $agendaFakultas->updated_at->diffForHumans() }}.
            </div>
